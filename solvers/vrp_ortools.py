...
class VRPOrToolsSolver(Solver):
    ...
    def update_runtime_throttle(self, obs: dict, replan_elapsed: float) -> None:
        N = obs["N"]
        G = len(obs["orders"])
        target_replan_time = 0.03 + 0.00003 * G + 0.00001 * (N * N)
        target_replan_time = max(0.04, min(0.25, target_replan_time))
    ...
    def run(self) -> dict:
        ...
        return self.env.result("VRPOrToolsSolver", time.time() - start)

    def get_sssp_path(self, source: Cell, goal: Cell, grid: List[List[int]]) -> List[Cell]:
        if source not in self.sssp_parent:
            dist = {source: 0}
            parent = {source: None}
            q = deque([source])
            while q:
            ...
            self.sssp_parent[source] = parent
            self.sssp_dist[source] = dist

        parent = self.sssp_parent[source]
    ...
    def get_sssp_dist(self, source: Cell, goal: Cell, grid: List[List[int]]) -> int:
        if source not in self.sssp_dist:
            _ = self.get_sssp_path(source, goal, grid)
        return self.sssp_dist[source].get(goal, self.INF)
                ...
                for oid in virtual_bag:
                    ...
                    if o:
                        ...
                        if d < self.INF:
                            ...
                            if est_t > o.et:
                                lateness_penalty = -250.0 * (est_t - o.et)
                            else:
                                slack = o.et - est_t
                                if slack <= 30:
                                    lateness_penalty = 100.0 * (30 - slack)
                    ...
                    for d_pickup, o in active_pickups[:K_next]:
                        ...
                        if d_delivery < self.INF:
                            ...
                            for bag_oid in virtual_bag:
                                ...
                                if bag_o:
                                    ...
                                    if est_t_without <= bag_o.et and est_t_with > bag_o.et:
                                        ...
                                        delay_cost += 200.0 * (est_t_with - est_t_without)
                            ...
                            if est_t_delivery > o.et:
                                lateness_penalty = -250.0 * (est_t_delivery - o.et)
                            else:
                                slack = o.et - est_t_delivery
                                if slack <= 30:
                                    lateness_penalty = 50.0 * (30 - slack)
    ...
    def plan_step(self, obs: dict) -> Dict[int, Tuple[str, int]]:
        ...
        if emergency_mode:
            ...
            L_max = max(3, base_L_max - penalty)
            L_max = max(3, min(12, L_max))
            K_prune = max(25, base_K_prune - 10 * penalty)
            K_prune = max(25, min(100, K_prune))
            K_next = max(5, 10 - 2 * penalty)
        ...
        try:
            ...
            tuned_wait_score = -500.0 if N >= 40 else -200.0
            ...
            for s in shippers:
                if s.id in immediate_targets:
                    ...
                    candidate_routes[s.id] = self.build_sequential_route(
                        s,
                        primary_tgt,
                        obs,
                        dist_penalty_coeff,
            ...
            for s in shippers:
                if s.id in immediate_targets:
                    ...
                    fallback_routes[s.id] = self.build_sequential_route(
                        s,
